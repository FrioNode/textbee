...
export class BillingService {
  ...
  private async resolvePlanChange({
    ...
    if (currentPlan?.name?.startsWith('custom')) {
      throw new BadRequestException({
        message:
          'You are on a custom plan, please contact billing@cloudtext.frionode.online to change your plan',
    ...
    if (
      ...
      throw new BadRequestException({
        message: `You are already on ${planName} plan, please contact billing@cloudtext.frionode.online to get a custom plan`,
  ...
  async changePlan({ user, payload }: { user: any; payload: any }) {
    ...
    try {
      ...
      if (statusCode === 403) {
        throw new BadRequestException({
          message:
            'Your subscription is canceled or ending and cannot be changed. Please resume it in the customer portal or contact billing@cloudtext.frionode.online.',
      ...
      if (statusCode === 409) {
        throw new BadRequestException({
          message:
            'A plan change is already in progress for your subscription. Please try again later or contact billing@cloudtext.frionode.online.',
          code: 'PENDING_UPDATE',
        })
      }
      throw new BadRequestException({
        message:
            'Failed to change plan, please try again or contact billing@cloudtext.frionode.online',
